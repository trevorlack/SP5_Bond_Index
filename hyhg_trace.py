...
for file in list(glob.glob("HY_Citi_Index*")):
    ...
    if file_date == next_day:
        ...
        as_of_date = [dt.datetime(*xlrd.xldate_as_tuple(item, 0)) for item in as_of_date]
        ...
        df = df[df.par > 0]
        df = df.drop(['glic', 'par', 'price'], axis=1)

        hyhg_blp_CUSIPs = '/CUSIP/' + df['Index_CUSIP'] + '@TRAC'

        blp = BLPInterface()
        blp_Bloomberg_IDs = pd.DataFrame(blp.referenceRequest(hyhg_blp_CUSIPs, ['ID_CUSIP', 'TRADE_STATUS', '144A_FLAG',
        ...
        #Date bloomberg historical request
        blp_date = datetime.strptime(next_day, '%Y-%m-%d').strftime('%Y%m%d')

        #High Yield Block Trade Criteria is 1Mil.  Blp code is 'TRL1'
        hyhg_CUSIP_Block = '/CUSIP/' + blp_Bloomberg_IDs['ID_CUSIP'] + '@TRL1'
        blp_BLOCK_Volume = blp.historicalRequest(hyhg_CUSIP_Block,
                                                 ['TRACE_NUMBER_OF_TRADES'], blp_date,
                                                 blp_date)  # 'PX_VOLUME', 'RSI_14D', 'PX_BID', 'PX_ASK'
        blp_BLOCK_Volume = pd.DataFrame(blp_BLOCK_Volume.unstack())
        blp_BLOCK_Volume = blp_BLOCK_Volume.reset_index()
        blp_BLOCK_Volume = blp_BLOCK_Volume.rename(columns={0: 'TRACE_Block_Trades', 'Security': 'ID_CUSIP'})
        blp_BLOCK_Volume = blp_BLOCK_Volume[['ID_CUSIP', 'TRACE_Block_Trades']]
        blp_BLOCK_Volume['ID_CUSIP'] = blp_BLOCK_Volume['ID_CUSIP'].str[7:16]
        blp_BLOCK_Volume.reset_index()
        ...
        #Add the Block trades
        hyhg_market_data = pd.merge(hyhg_market_data, blp_BLOCK_Volume, on=['ID_CUSIP'], how='outer')
        hyhg_market_data = hyhg_market_data.fillna(value=0)

        TRACE_Block_Trades = hyhg_market_data['TRACE_Block_Trades'].tolist()
        TRACE_Block_Trades = [str(x) for x in TRACE_Block_Trades]
        ...
        a144 = hyhg_market_data['144A_FLAG'].tolist()
        ...
        cursor = db.cursor()
        values = "REPLACE INTO hyhg_liquidity (Date, Volume, ID_CUSIP, Index_CUSIP, TRADE_STATUS, 144a, Bid, Ask, TRACE_Trades, recovery_rate, TRACE_Block_Trades) \
                   VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"

        params = [(str(Date[i]), str(Volume[i]), str(ID_CUSIP[i]), str(Index_CUSIP[i]),
                   str(TRADE_STATUS[i]), str(a144[i]), str(Bid[i]),
                   str(Ask[i]), str(TRACE_Trades[i]), str(recovery_rate[i]), str(TRACE_Block_Trades[i])) for i in range(length)]
